	...
	switch ( event.keyCode ) {
		...
		case 48: //0 --> shoot poop
			...
			if(lookingAtSomeone != -1){
				...
				// distV.copy( controls.position() );

				// var toWhomPoistion = dailyLifePlayerDict[ lookingAtSomeone ].player.position.clone();
				// toWhomPoistion.subVectors( toWhomPoistion, distV ).multiplyScalar(5/6).add( distV );				
				// toWhomPoistion = firstGuy.player.worldToLocal( toWhomPoistion );

				// var shootT = distV.distanceTo( toWhomPoistion );

				// firstGuy.player.children[2].visible = true;
				// firstGuy.player.children[2].lookAt( dailyLifePlayerDict[ lookingAtSomeone ].player );

				// var mpOut = new TWEEN.Tween(firstGuy.player.children[2].position)
				// 							.to( {x: toWhomPoistion.x,
				// 								  y: toWhomPoistion.y-1,
				// 								  z: toWhomPoistion.z}, Math.floor(shootT)*300 )
				// 							.easing( TWEEN.Easing.Back.InOut );
				
				// var mpBack = new TWEEN.Tween(firstGuy.player.children[2].position)
				// 							.to( {x: 0, y: 0, z: 0}, Math.floor(shootT)*300 )
				// 							.easing( TWEEN.Easing.Back.InOut )
				// 							.delay(1000)
				// 							.onComplete(function(){
				// 								firstGuy.player.children[2].visible = false;
				// 							});
				// mpOut.chain(mpBack);
				// mpOut.start();
