...
function superInit(){
	...
	// wave
		...
		// physijs
			// var water_material = Physijs.createMaterial(
	...
	*/
	...
	// WINDOW
		windowTex = THREE.ImageUtils.loadTexture('images/windowS.png');
...
function createPoop( _pos ) {
	...
	poopVector = controls.getDirection().multiplyScalar( 20 );
...
function myKeyPressed( event ){
	switch ( event.keyCode ) {
		...
		case 85: //u
			...
			// v.2
			var newPos = {};
			newPos.x = [toiletCenters[meInBGroup].x,0,0,0,0];
			newPos.y = [-17,-16,-22,-23,-50];
			newPos.z = [toiletCenters[meInBGroup].z,-13,-10,-1,-1];
			controls.createTweenForMove( newPos, 25000 );
			setTimeout(function(){
